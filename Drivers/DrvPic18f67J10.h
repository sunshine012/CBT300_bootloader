...
extern volatile near union {
  ...
  struct {                 // This structure was added to access the
    unsigned ADCS: 3;      // ADCS and ACQT bits as bit group and not
    unsigned ACQT: 3;      // as seperate bits, CHE 08-31-2015.
  };                        //
} ADCON2bits;
...
extern volatile near union {
  ...
  struct {                 // This structure was added to access the
    unsigned PCFG: 4;      // PCFG and VCFG bits as bit group and not
    unsigned VCFG: 2;      // as seperate bits, CHE 07-08-16.
  };                        //
} ADCON1bits;
...
extern volatile near union {
  ...
  struct {                 // This structure was added to access the 
    unsigned :2;           // channel selection bits as group of 4
    unsigned CHS:4;        // bytes. CHE 07-08-16.
  };                       //
} ADCON0bits;
...
extern volatile near union {
  ...
  struct {                       // This structure was added to access the 
    unsigned T2CKPS: 2;          // post and pre scaler bits as bit groups.
    unsigned :1;                 // CHE 07-08-20.
    unsigned T2OUTPS: 4;         //
  };                             //
} T2CONbits;
...
extern volatile near union {
  ...
  struct {                       // This structure was added to access the 
    unsigned :4;                 // post scaler bits as bit groups.
    unsigned T1CKPS: 2;          // CHE 07-08-20.
  };                             //
} T1CONbits;
...
extern volatile near union {
  ...
  struct {                     // This structure was added to access the 
    unsigned T0PS: 3;          // pre scaler bits as bit group.
  };                           // CHE 07-08-20.
} T0CONbits;
